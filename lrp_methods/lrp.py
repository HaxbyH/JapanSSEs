...
from lrp_methods.utils import layers_lookup
...
class LRPModel(nn.Module):
    ...
    def _create_lrp_model(self) -> torch.nn.ModuleList:
        ...
        # print("Look up table: ", lookup_table)

        # Run backwards through layers
        for i, layer in enumerate(layers[::-1]):
            # print(f"{i} - {layer.__class__}, {layer.__class__.__module__}")
            try:
                layers[i] = lookup_table[layer.__class__](layer=layer)
            except KeyError:
                message = f"Layer-wise relevance propagation not implemented for " \
                          f"{layer.__class__.__name__} layer." + f"\n\n. {layer.__class__}"
                print(f"Error: not match for {layer.__class__}, using identity layer instead.")
                raise NotImplementedError(message)
                # layers[i] = RelevancePropagationIdentity
    ...
    def _get_layer_operations(self) -> torch.nn.ModuleList:
        ...
        layers.insert((len(layers)-1), torch.nn.modules.flatten.Flatten())
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        ...
        # print(relevance.shape, "REL SHAPE")

        return relevance.squeeze().detach().cpu()
